...
package_root="$(mktemp -d "${TMPDIR:-/tmp}/cgrok-deb.XXXXXX")"
trap 'rm -rf "$package_root"' EXIT
chmod 0755 "$package_root"

install -d -m 0755 "$package_root/DEBIAN" "$package_root/usr/bin"
install -m 0755 "$binary" "$package_root/usr/bin/cgrok"
...
Description: Cloudflare-based secure tunneling CLI
CONTROL

install -d -m 0755 "$output_dir"
dpkg-deb --build --root-owner-group \
  "$package_root" \
  "$output_dir/cgrok_${version}_${arch}.deb"
